...
The merge-ready Git-causality review checks the already-merged upstream contract, not the future merge of this Security review. Workspace Governance PR #187 used fetched base `1b7952ba180a3a52a997253e840b769574efd632`, reviewed head `3167748e48cb9e68b60fe83a9a3f55bfbfd7a3b3`, and squash-merge commit `eb9ec14e9191535a358bc80418aa0a14ad8b8609`. The version-2 closure contract and schemas are read from that exact merge commit. A real-Git positive check must prove the provider-reported PR head and merge, the merge's fetched-base parent, and byte-identical versioned closure files at the reviewed head and merged revision. It must not assume ancestry across a squash merge. Negative checks must reject a mismatched reviewed head or absent merge/source evidence. These checks establish the causality rule for the contract under review. The merge of this Security PR is separate, later source-landing evidence and is not a prerequisite for its own merge-ready Review Packet.

On 2026-09-12, GitHub PR readback reported PR #187 `MERGED` with the head and merge commits above. In `workspace-governance`, `git rev-parse <merge>^` returned the fetched base; `git diff --exit-code <reviewed-head> <merge> --` the closure contract and request, history-event, Studio-readback, and receipt schemas exited `0`, and `git ls-tree` found all five files at the merge. Replacing the reviewed head with the fetched base made the same file comparison exit `1`; querying an absent merge commit with `git cat-file -e` exited `128`. These are real-Git contract-review checks, not a claim that this Security PR or any Studio transition has merged.
